 ...
 *
 * This keeps terminal backends independent from escape-sequence fragmentation: reads may split CSI
 * keys or bracketed paste markers across arbitrary chunks.
 */
final class TerminalInputBuffer:
  ...
  private def completeCsiLength(value: String): Option[Int] =
    if value.length < 3 then None
      ...
      while i < value.length && result.isEmpty do
        ...
        if ch >= 0x40 && ch <= 0x7e then result = Some(i + 1)
        i += 1
      result
  ...
  private def completeTerminatedLength(value: String): Option[Int] =
    ...
    var result = Option.empty[Int]
    while i < value.length && result.isEmpty do
      if value.charAt(i) === '\u0007' then result = Some(i + 1)
      else if (value.charAt(i) === Esc) && i + 1 < value.length && (value.charAt(
          i + 1
        ) === '\\')
      then result = Some(i + 2)
      i += 1
    result
