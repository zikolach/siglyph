...
final case class TerminalCapabilities(trueColor: Boolean, hyperlinks: Boolean, images: Option[ImageProtocol]) derives CanEqual
...
object TerminalCapabilities:
  def detect(env: Map[String, String] = sys.env, tmuxForwardsHyperlinks: => Boolean = false): TerminalCapabilities =
    ...
    if env.contains("TMUX") || term.startsWith("tmux") then
      TerminalCapabilities(trueColor = trueColorHint, hyperlinks = tmuxForwardsHyperlinks, images = None)
    ...
    else if termProgram == "ghostty" || term.contains("ghostty") || env.contains("GHOSTTY_RESOURCES_DIR") then
      TerminalCapabilities(trueColor = true, hyperlinks = true, images = Some(ImageProtocol.Kitty))
    ...
    else if env.contains("WT_SESSION") || termProgram == "vscode" || termProgram == "alacritty" then
