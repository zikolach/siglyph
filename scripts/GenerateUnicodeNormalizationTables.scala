...
object GenerateUnicodeNormalizationTables:
  ...
  @main def main(outputs: String*): Unit =
    ...
    val lowercaseMappings = records.flatMap { fields =>
      val value = fields(13)
      Option.when(value.nonEmpty) {
        Integer.parseInt(fields(0), 16) -> Integer.parseInt(value, 16)
      }
    }
...
/** Generated Unicode $Version canonical decomposition, combining-class, and lowercase data. */
...
${array("lowercaseKeys", lowercaseMappings.map(_._1))}
${array("lowercaseValues", lowercaseMappings.map(_._2))}
