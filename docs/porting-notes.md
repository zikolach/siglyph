...
- Editor input is typed. `scala-tui` does not emulate `pi-tui`'s raw standalone-backslash Enter workaround; newline behavior is represented by `EditorEnterBehavior` and typed Enter keybindings.
- The editor does not expose `pi-tui`'s `insertTextAtCursor` API. Applications use typed input, paste input, or `setText` instead.
...
- `pi-tui` can auto-apply a single force-file autocomplete suggestion. `scala-tui` keeps autocomplete selection explicit and lets providers insert replacement text through the same selection path as other suggestions.
