...
`alternate-screen-maven.scala` demonstrates opt-in alternate-screen mode with a practical Sonatype Central explorer. It searches the Sonatype Central browse API, shows artifacts, versions, published dates, and build-tool snippets in a temporary full-screen workspace, copies the selected snippet through terminal clipboard escape sequences, and exits back to the normal shell without leaving the search UI in scrollback. It uses `jsoniter-scala` only inside the example for JSON parsing. Until alternate-screen mode is available in a published release, this demo uses local source directives for `core/src` and `terminalJvm/src`, so run it from this checkout.
