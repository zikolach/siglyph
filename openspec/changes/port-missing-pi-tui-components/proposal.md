...
- Extend `Input` and `Editor` behavior to match upstream editing interactions (undo, kill-ring, word navigation/deletion, and related command semantics).
...
- Implement a markdown rendering path under the existing pluggable `markdown` module and render markdown as a normal component.
- Add image component/protocol support for Kitty/iTerm2 escape image emission with clear fallback behavior when unsupported.
...
- `text-editing`: Extend single-line and multiline editing requirements with undo/kill-ring/word navigation semantics, large-paste markers, and IME/hardware-cursor behavior.
- `autocomplete`: Add combined slash + file completion provider and deterministic path-trigger parsing.
- `markdown-rendering`: Move from placeholder/pluggable structure to concrete behavior for rendered markdown output and width-safe fallback behavior.
...
- `developer-api`: Expand public component and helper APIs to expose the new editing/parsing/media capabilities without adding runtime dependencies.

## Impact

- **Code areas:** `core/src/scalatui/components`, `core/src/scalatui/autocomplete`, `core/src/scalatui/editing`, `core/src/scalatui/terminal`, `core/src/scalatui/core`, `core/src/scalatui/syntax` (as needed), `markdown/src`, demos, and tests.
...
- **Dependencies:** No new runtime dependencies; keep runtime dependency-light and maintain JVM + Scala Native compatibility.
