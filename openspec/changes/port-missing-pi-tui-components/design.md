...
3. **Add a combined autocomplete model in `autocomplete` rather than embedding path logic in `Editor`**
...
4. **Reuse existing `TerminalInput`/`TerminalInputParser` and add focused normalization utilities for editing commands**
...
5. **Implement image protocol emission conservatively (capability-gated), with readable fallback text on unsupported terminals**
...
2. Implement `Input` and `Editor` parity behaviors while retaining existing control flow and callback contracts.
...
4. Add markdown component behavior and image component/protocol behavior plus fallback tests.
...
- For undo semantics, should an undo step include all insert/delete mutations or only high-level edit commands (`undo`, `redo`, `yank`, `yank-pop`)?
