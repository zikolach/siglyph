...
- Invoke recovery only after a resize callback invalidates committed geometry; redundant same-size notifications do not recover or clear the viewport. Keep coalesced invalidation when an intermediate resize changes geometry and a later callback returns to the committed dimensions. A geometry delta detected without a resize callback still clears stale rows but does not invoke recovery. Render and validate the retained live frame first, reserve its physical rows (including one cursor anchor for an empty frame), and ask the provider for current-width durable tail lines only when a strict budget bounded by both previous and current viewport capacities is positive. Expose previous geometry/capacity so the application can select the old semantic tail before reflowing it.
